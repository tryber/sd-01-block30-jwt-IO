...
async function readFolderImages(fileName = '') {
  ...
  return fileJson.includes(`${fileName}.png`);
}
...
async function imageValidMiddleware(req, res, next) {
  ...
  if (fieldname !== 'image' || !imageTextValid(originalname))
    return res.status(422).json({ message: 'File is not document .jpg or .png' });

  if (await readFolderImages(productId)) return res.status(409).json({ message: 'File already exists' });
